...
- [ ] **1.1 Define Custom `iced::Theme` or Extend Default:**
    - [ ] Create a new module `super_clicker/src/theme.rs`.
    - [ ] Within `theme.rs`, define a custom `AppTheme` enum that wraps `iced::Theme` or creates a new theme struct.
    - [ ] Implement `From<AppTheme> for iced::Theme` to allow seamless use.
    - [ ] Implement the `iced::widget::container::StyleSheet` trait for `AppTheme` to define card-like backgrounds for grouped controls (e.g., `PrimaryControlsContainer`, `HotkeyConfigContainer`).
    - [ ] Implement `iced::widget::button::StyleSheet` for `AppTheme` to define button styles for `PrimaryButton` (Start/Stop), `StatusButton` (Status Display), and `IconButton` (if separate icon-only buttons are needed).
    - [ ] Implement `iced::widget::text_input::StyleSheet`, `iced::widget::pick_list::StyleSheet`, `iced::widget::checkbox::StyleSheet` for `AppTheme` to ensure all form elements are styled using tokens and handle disabled states.
    - [ ] **Define Color Design Tokens:**
        - [ ] Within `theme.rs` (or a sub-module like `theme::colors`), explicitly define constants or functions returning `iced::Color` for:
    ...
    - [ ] **Define Spacing Design Tokens:**
        - [ ] Define constants for consistent padding and spacing values (e.g., `SPACING_SM`, `SPACING_MD`, `SPACING_LG`).
- [ ] **1.2 Update `app.rs` for Custom Theme Integration:**
    - [ ] Modify `SuperClicker::new` to initialize `current_theme` with `AppTheme::Light` or `AppTheme::Dark` based on `dark_light::detect()`.
    - [ ] Update `SuperClicker::theme` to return the `iced::Theme` derived from `self.current_theme: AppTheme`.
    - [ ] Update `Message::ThemeChanged` to accept and set `AppTheme`.
