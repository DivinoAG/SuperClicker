...
## Tasks / Subtasks
- [ ] Add "Start" and "Stop" buttons to `ui.rs`.
- [ ] Implement `Message` variants for Start and Stop button clicks in `app.rs`.
- [ ] Implement enable/disable logic for the buttons based on `is_running` state.
- [ ] Integrate button click messages with `clicking_engine` in `app.rs`.
